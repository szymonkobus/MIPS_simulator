...
for file_name in sorted(files):
    ...
    if(file_exists(p_tests_binary + TestId + ".bin")):
        exit = sps.call([simulator, p_tests_binary + TestId + ".bin"], stderr=sps.PIPE, stdout = sps.PIPE)
...
for file_name in sorted(files_IO):
    ...
    if(file_exists(p_tests_binary + TestId + ".bin")):
        ...
        if IO_char == 'EOF':
            ...
            sim.communicate(IO_char.encode('iso8859-1'))
            exit = sim.returncode
            ...
            IO_char_out = (stdout.decode('iso8859-1'))[0]
            exit = sim.returncode
